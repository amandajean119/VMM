...
int crossover(ga * ga){
	...
	int satisfies = -1;
	machine * newPopulation = (machine*) malloc(POPULATION*sizeof(machine));
        newPopulation[0] = ga->population[0]; 
	machine * p1;
	machine * p2;
	machine * ch1;
	machine * ch2;
	...
	while(nrNewPop < POPULATION - 1){
		...
		if((p1->id != newPopulation[0].id) && (p2->id != newPopulation[0].id)){  
		// Children pointers
...
int mutation(ga * ga){
	...
	for (i = 1; i < POPULATION; i++) {
	    ...
	    if(indiv->id != ga->bestMachineIndex){ 
...
int main(){
	...
	remove("fitnessPalaciosTest.txt");
	remove("chromosomePalaciosTest.txt");
	
	fitnessFile = fopen("fitnessPalaciosTest.txt", "w");
